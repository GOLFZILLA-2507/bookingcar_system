...
$whereParts = ["start_date BETWEEN ? AND ?", "booking_status = ?"];
$params = [$startDate, $endDate, 'อนุมัติแล้ว'];
...
$sql = "SELECT id, full_name, destination, start_date, end_date, start_time, end_time, additional_details, booking_status,
        car_id, car_name, license_plate, companions,
...
<div class="container py-4">
  ...
  <form class="row g-3 mb-3" method="get">
    ...
    <div class="col-md-3 d-flex justify-content-between">
      <a href="?month=<?= $month - 1 <= 0 ? 12 : $month - 1 ?>&year=<?= $month - 1 <= 0 ? $year - 1 : $year ?>" class="btn btn-outline-secondary">&laquo; เดือนก่อนหน้า</a>
      <a href="?month=<?= $month + 1 > 12 ? 1 : $month + 1 ?>&year=<?= $month + 1 > 12 ? $year + 1 : $year ?>" class="btn btn-outline-secondary">เดือนถัดไป &raquo;</a>
    </div>
  </form>
                  ...
                  <div class="modal-content">
                    ...
                    <div class="modal-body">
                      <dl class="row">
                        ...
                        <dt class="col-sm-3">วันที่เดินทาง</dt><dd class="col-sm-9"><?= date('d/m/Y H:i', strtotime($b['start_date'] . ' ' . $b['start_time'])) ?> - <?= date('d/m/Y H:i', strtotime($b['end_date'] . ' ' . $b['end_time'])) ?></dd>
                        ...
                        <dt class="col-sm-3">รถที่ได้</dt>
                          <dd class="col-sm-9">
                            <?= $b['car_id'] ? htmlspecialchars($b['car_name'] . '  ' . $b['license_plate'] . '') : 'ยังไม่ได้จัดสรร' ?>
                          </dd>
                        <dt class="col-sm-3">ผู้ร่วมเดินทาง</dt><dd class="col-sm-9"><?= htmlspecialchars($b['companions']) ?></dd>
                        <dt class="col-sm-3">สถานะ</dt><dd class="col-sm-9"><span class="badge bg-success">อนุมัติแล้ว</span></dd>
                      </dl>
                    </div>
