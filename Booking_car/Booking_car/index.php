...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT EmployeeID, password, fullname, role_car FROM Employee WHERE EmployeeID = :EmployeeID AND active = 1");
        ...
        elseif (!password_verify($password, $user['password'])) {
            ...
            $_SESSION['role_car'] = $user['role_car'];

            // เปลี่ยนเส้นทางไปยังหน้า dashboard
            if($user['role_car']=='admin'){ 
